...
const { computeStreaks, computeCircadian, computeAchievements, predictBurn } = require('./stats');
...
async function buildData() {
  ...
  const tokens = {
    ...
    reasoning: (codex.tokens.reasoning || 0) + (claude.tokens.reasoning || 0),
  };
  ...
  for (const src of [codex.byModel, claude.byModel]) {
    for (const [model, v] of Object.entries(src)) {
      const t = (byModel[model] = byModel[model] || {
        tokens: 0, input: 0, output: 0, cached: 0, cacheWrite: 0, reasoning: 0, count: 0,
      });
      t.tokens += Number(v.tokens) || 0;
      t.input += Number(v.input) || 0;
      t.output += Number(v.output) || 0;
      t.cached += Number(v.cached) || 0;
      t.cacheWrite += Number(v.cacheWrite) || 0;
      t.reasoning += Number(v.reasoning) || 0;
      t.count += Number(v.count) || 0;
    }
  }

  const daily = mergeDaily(codex.daily, claude.daily);
  const hourly = mergeDaily(codex.hourly, claude.hourly);
  const mergedTasks = [...codex.tasks, ...claude.tasks];

  // 价值估算（羊毛进度）
  let usd = 0;
  for (const [model, v] of Object.entries(byModel)) {
    usd += prices.estimateUsd(model, v.input, v.cached, v.cacheWrite, v.output);
  }
  ...
  for (const [model, v] of Object.entries(byModel)) {
    if (v.tokens > 0) {
      unitPrice[model] = prices.estimateUsd(model, v.input, v.cached, v.cacheWrite, v.output) / v.tokens;
    }
  }
  const dayValue = (ymd) => {
    let sum = 0;
  ...
  const projectCountAll = Object.keys(projects).length;
  ...
  const modelRanking = Object.entries(byModel)
    .map(([model, v]) => ({
      ...
      tokens: v.tokens,
      count: v.count,
      usd: Math.round(prices.estimateUsd(model, v.input, v.cached, v.cacheWrite, v.output) * 100) / 100,
    }))
  ...
  // 燃烧速率预测：为每个窗口附加 burn 字段（耗尽时刻估算）
  quota.windows = predictBurn(quota.windows, Date.now());

  // ===== 原创玩法：连击 / 生物钟 / 成就 =====
  const streaks = computeStreaks(dailySeries);
  const circadian = computeCircadian(hourly);
  ...
  return {
    ...
    stats: { streaks, circadian, achievements },
  };
}
